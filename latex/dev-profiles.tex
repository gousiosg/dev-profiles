...
\emph{named entity recognition} in combination with \emph{named entity
disambiguation}. Given an unstructured text (e.g. a job advertisement) named
entity recognition determines which words or phrases in the text refer to an
entity(-ies) in a given predefined set of entities. Named entity disambiguation
determines which concrete entities a particular word or phrase refers to.  The
combination of both techniques have been repeatably shown~\claudia{Ref?} to be
powerful tools to turn natural language text into a structured representation
...
We use DBPedia Spotlight,\footnote{\url{http://dbpedia-spotlight.github.io/}}
one of the most commonly used open-source annotation toolkits for natural
language text, as a concrete implementation for concept extraction. The
large-scale DBpedia ontology\claudia{ref?} behind the DBPedia Spotlight service is
automatically derived from Wikipedia and based on the English Wikipedia edition,
currently contains more than 4.5 million elements (``things'') and nearly 600
million links between them. Wikipedia (and thus DBPedia) contains entries
covering most if not all programming languages, many important programming
...
``Java''). Each entity is uniquely identified as a particular LOD concept.
...
relatedness between any two concepts. Since in our particular use
...
wealth of information that the developers should be familiar with they own a
...
projects the developer is \emph{associated} with.  Here, association can either
mean a repository the developer created or forked.

\subsection{Concept weighting}

The extracted concepts from job descriptions and GitHub profiles are converted
...
equally important. We use the well-known TF.IDF \claudia{ref? Also, one sentence
on how TF.IDF works for weighting} weighting scheme
to determine non-binary concept weights (benefiting concepts which occur rarely
across the entire corpus of documents, but often within particular documents);
these weights are computed separately for the corpus of job advertisements and
the corpus of developer profiles. Additionally, we enrich the concept vectors by
adding for each found concepts up to $t$ semantically related concepts based on
a particular relatedness measure \claudia{is this through the ``random walks''?
I do not understand, please explain a bit more}.
