...
Finally we briefly discuss the results of ranking all developers for a single 
...
knowledge through README files in their profile repositories ii) the use of a
...
results, such as\claudia{?}, which leads us to believe that our approach is a
promising start. In the future, we plan to augment developer profiles through
extracting fine-grained information from the source code and build files for
their repositories.
